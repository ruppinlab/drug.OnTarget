...
write.csv(correlations_with_a_gene, "../data/Correlation_df_with_viagra.csv")
```
...
ggplot(predicted_rank_of_targets_of_viagra, aes(x=reorder(rownames(predicted_rank_of_targets_of_viagra), estimate.cor), 
                                  y= estimate.cor, 
                                  color= fdr_pvalue< 0.05))+
  geom_point(stat="identity", aes(size= -rank))+
  geom_label(label = predicted_rank_of_targets_of_viagra$rank, hjust = 0, nudge_x = 0.01, nudge_y = 0.001,size= 3)+
  theme(text = element_text(size = 9),element_line(size =1))+
  labs(x= 'Targets of sildenafil', 
...
plot1_drug_kill_df=data.frame(Drug_kill= DOI_response_matched, 
           mut= onTarget$mutations_matrix[TOI,colnames(crispr_matched)],
           exp= expression_hulala[TOI,])
ggplot(plot1_drug_kill_df, aes(x=exp, y= Drug_kill))+
  geom_point()+
  geom_smooth(method = "lm", se= F)+
  stat_cor(label.y.npc = "top")
```
```{r}
ggplot(plot1_drug_kill_df, aes(x=factor(mut), y= Drug_kill))+
  geom_boxplot()+
  stat_compare_means()
```
