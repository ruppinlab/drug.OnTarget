...
<!--Finding the pathway enrichment test with all drug: removing NAN and no targets one-->
```{r}
...
drugs_with_moa= drugs_with_moa[!(is.na(drugs_with_moa$target) | is.na(drugs_with_moa$moa)),]
```

<!--Finding the pathway enrichment test with all drug: No condition-->
```{r}
Known_Pathway_Enrichment=do.call(rbind, lapply(1:length(drugs_with_moa$moa), function(x)
  data.frame(rank=match(drugs_with_moa$moa[x], gsea_enrichment_all_drugs_sorted[[x]]$pathway),
             drugBroadID= names(gsea_enrichment_all_drugs_sorted[x]),
    gsea_enrichment_all_drugs_sorted[[x]][match(drugs_with_moa$moa[x], gsea_enrichment_all_drugs_sorted[[x]]$pathway),]
    )
  ))

### Adding drug broad id is adding 253 NA broad ID value: one reason could be repeatetion
# Known_Pathway_Enrichment= Known_Pathway_Enrichment %>% mutate(drugBroadID= drugs_with_moa$broad_id_trimmed[match(drugs_with_moa$broad_id_trimmed,names(gsea_enrichment_all_drugs_sorted))]) %>% relocate(drugBroadID, .after = rank)
Known_Pathway_Enrichment = Known_Pathway_Enrichment[order(Known_Pathway_Enrichment$rank, decreasing = F),]
Known_Pathway_Enrichment= Known_Pathway_Enrichment[!(is.na(Known_Pathway_Enrichment$rank)),]
hist(Known_Pathway_Enrichment$rank)


## best specisity threshold
threshold = 1295*523/nrow(onTarget$avana_22Q2)

Known_Pathway_Enrichment[Known_Pathway_Enrichment$rank < threshold,]
sum(Known_Pathway_Enrichment$rank < threshold)/ length(Known_Pathway_Enrichment$rank)
```
<!--Plot1: including all the genes without any condition-->
```{r fig.width=5,fig.height=5}
dfplot1_pathway_all= Known_Pathway_Enrichment
zoomed_part_df_with_text= dfplot1_pathway_all %>% filter(pval < 0.05 & NES > 0) %>% mutate( zoom = TRUE)
    
Figure_1= ggplot(dfplot1_pathway_all, aes(y= rank, x= NES, color= pval <0.05))+
  geom_point()+
  facet_zoom(x= pval < 0.05 & NES > 0, zoom.data = zoom)+
  geom_text_repel(data = zoomed_part_df_with_text, aes(label = str_wrap(paste(pathway,":",drugBroadID), 30)),
                  nudge_x = c(-1.5,1.5),
                  nudge_y = c(-10,-750), max.overlaps = 14,
                  size= 3)+
  scale_y_reverse()+
...
ggsave('../result/Pathway_enrichment_plot1_all.jpg', Figure_1,
