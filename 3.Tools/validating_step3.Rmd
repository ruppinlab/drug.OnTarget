...
step1_vs_step3<- function(seed_id=1, rank_threshold=2){
  ...
  set.seed(seed_id)
  negative_labels=sample(1:nrow(corrMat), size = length(correctly_predict_drugID))
  ...
  Sec_KnonwTarget_score_step1vs3=sapply(correctly_predict_drugID, function(x)
    ...
    err_handle(data.frame(drugName=x,
                          secKnownTarget_Step1=Sec_KnownTargets_list[[x]][which.max(corrMat_rank[Sec_KnownTargets_list[[x]], x])],
                          secKnownTarget_Step3=Sec_KnownTargets_list[[x]][which.max(sec_corrMat_rank[Sec_KnownTargets_list[[x]], x])],
                          SecKnown_Pred_rank_Step1=max(corrMat[Sec_KnownTargets_list[[x]], x], na.rm = T) ,
                          Control_Step1=corrMat[negative_labels[x], x],
                          SecKnown_Pred_rank_Step3=max(sec_corrMat[Sec_KnownTargets_list[[x]], x], na.rm = T),
                          Control_Step3=sec_corrMat[negative_labels[x], x],
                          Control=rownames(corrMat)[negative_labels[x]]
    ))
  )
  Sec_KnonwTarget_score_step1vs3_df=do.call(rbind, Sec_KnonwTarget_score_step1vs3)
...
S1vsS3_seed1=step1_vs_step3(seed_id = 1, rank_threshold = 2)
...
Figure6D <- ggplot(Score_from_Step3, aes(y=Score, x=label))+
  ...
  labs(y='Secondary DKS Score', x='Ground Truth Labels')
```
<!-- Figure 6E -->
```{r}
...
Figure6E <- ggplot(Score_from_Step1, aes(y=Score, x=label))+
  ...
  stat_compare_means(label = 'p')+
  theme_bw(base_size = 15)+
  labs(y='DKS Score', x='Ground Truth Labels')
```
<!-- Figure 6F -->
```{r}
...
Figure6F <- ggplot(Score_from_Step13_combination, aes(y=Score, x=label))+
  ...
  stat_compare_means(label = 'p')+
  theme_bw(base_size = 15)+
  labs(y='Max of DKS and Secondary-DKS score', x='Ground Truth Labels')
```
Plot all AUC together
```{r}
secTargetROC_list=lapply(S1vsS3_seed1[4:6], smooth)
secTargetROC_list[1:2]
# Add mean AUC
names(secTargetROC_list)=c('Step1=0.85', 'Step3=0.92', 'Step1+3=0.85')

Figure6G <- ggroc(secTargetROC_list[1:2])+ 
...
```
```{r}
Figure6_row2 <-grid.arrange(Figure6D, Figure6E, Figure6G,
                       layout_matrix = rbind(c(1, 2, 4)) )
ggsave(Figure6_row2, filename = '/Users/sinhas8/Project_TrueTarget/Results/Figure6_row2.png',
       width = 10, height = 4)
ggsave(Figure6_row2, filename = '/Users/sinhas8/Project_TrueTarget/Results/Figure6_row2.pdf',
       width = 10, height = 4)
```
