...
roc_platinum=smooth(roc(platinum_dataset$Label, platinum_dataset$crispr_corr))
...
names(roc_overall_list)=c('All data points=0.80', 'Platinum Standard=0.89')
Figure1D <- ggroc(roc_overall_list)+ 
  theme_bw(base_size = 18) + 
  theme(legend.title = element_blank(), 
...
Figure1E<- ggplot(Figure1E_df2plot, aes(x=rank_Threshold, y=Specificity) )+
...
```{r}
Figure1BE=ggarrange(Figure1A, Figure1B, Figure1D, Figure1E, nrow = 2, ncol=2, labels = 'AUTO', 
                    font.label = list(size = 26))

ggsave(Figure1BE, filename = '/Users/sinhas8/Project_TrueTarget/Results/Figure1BE_v0.png',
       width = 15, height = 12)
ggsave(Figure1BE, filename = '/Users/sinhas8/Project_TrueTarget/Results/Figure1BE_v0.pdf',
       width = 15, height = 12)
```
#Diamond dataset
```{r}
...
Comp_Gold_Standard_df[grep('cobimetinib',Comp_Gold_Standard_df$Chemical.Name, ignore.case = T),]
```
<!-- V8: Hihgly Selective inhibitors based on their binding profile -->

```{r}
# Preprocessing
highly_selective_inhibitor=readxl::read_xlsx('/Users/sinhas8/Project_TrueTarget/Data/20220419-L3500-Highly-Selective-Inhibitor-Library-96-well.xlsx', sheet = 2)
...
test_AUC_A8<-function(seedNumber=1, numberOfTargets_Thr=5){
...
V8_ROC=test_AUC_A8(seedNumber=1, numberOfTargets_Thr=30)
dim(V8_ROC[[2]])
V8_ROC[[1]]
# Area under the curve: 0.9527
