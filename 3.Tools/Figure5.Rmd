...
common_colnames=Reduce(intersect, list(colnames(avana_matched),
                                       colnames(drug_matched),
                                       colnames(onTarget$mutations_matrix)))

DOI_id=onTarget$drugCategory$broad_id_trimmed[match_stripall(DOI, onTarget$drugCategory$name)]
Figure5J_data=data.frame(crispr=avana_matched[GOI,common_colnames],
                         drug_response=drug_matched[DOI_id,common_colnames],
                         mutation=onTarget$mutations_matrix[GOI,common_colnames])
...
```{r}
corr_PriTarget=KnownTarget_predictions$Maxcorr>0.23 & KnownTarget_predictions$mutation_interaction_P<0.2
KnownTarget_predictions_CorrPrim=KnownTarget_predictions[corr_PriTarget,]

na.omit(KnownTarget_predictions_CorrPrim[order(KnownTarget_predictions_CorrPrim$mutation_interaction_strength),
                                 c('drugName','MaxTargetName',
                                   'mutation_interaction_strength', 'mutation_interaction_P')])
```
...
Figure5K <- ggplot(Figure5J_data, aes(x=crispr, y=drug_response, color=factor(mutation)))+
  ...
  stat_smooth(method = 'lm', se = T)+
  theme_bw(base_size = 12)+
  labs(x='Viability after MDM2 CRISPR-KO',
       y='Response to CGM097',
       color='MDM2\nmutation\nstatus')+
  stat_cor(label.y = c(0.8,0.9), size=3)
Figure5K
```
...
```{r}
Figure5JK <- grid.arrange(Figure5J, Figure5K)
ggsave(Figure5JK, filename = '/Users/sinhas8/Project_TrueTarget/Results/Figure5JK.pdf',width=4,height = 8)
```
