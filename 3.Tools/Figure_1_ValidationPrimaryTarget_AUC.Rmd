---
title: "Figure 1: validation of primary target identification"
output: html_notebook
---

## Figure 1: Overview of our pipeline and performance across eight gold-standard datasets.

#### Tested TrueTarget in eight independent gold standard datasets comprising high-confidence drug-target pairs collected from diverse sources. These datasets include drug-target pairs that have 
...
###########################################################################
# Subseting and showing signal from DrugBank
...
<!-- From drugbank  -->
<!-- 1) V2A_ROC: focusing on Antagonist; 
     2) V2B_ROC: focusing on Inhibitor: -->
```{r}
...
test_AUC_V2<-function(seedNumber = 1,
  ...
  Complete_Set$crispr_corr = sapply(1:nrow(Complete_Set), function(x)
    err_handle(corrMat_rank[as.character(Complete_Set$Drugbank_Gene)[x], as.character(Complete_Set$drug_BroadID)[x]]))
...
# OnlY focusing on Inhibitor drugs V2B (0.69)
...
sab_score_matched_trimmed$Targets[grep('BRAF',sab_score_matched_trimmed$Targets)] = 'BRAF' ## also checked for 'IDH1' & 'IDH2'
...
df2plot_Figure1A$Dataset_names = paste(df2plot_Figure1A$Dataset_names, '(N = ',c(16, 28, 
                                                                             86,
                                                                             24, 28, 90, 52, 142), ')', sep = '')

## Alternative Figure 1C: Bar plot
...
<!-- # Nameing the ROC list to create an annotation in the ROC AUC plot -->
```{r}
names(ROC_list) = c(paste('COSMIC resistance= ', round(ROC_list$COSMIC_resistance$auc[1],2)),
                  paste('oncoKB resistance= ', round(ROC_list$oncoKB_resistance$auc[1],2)),
                  paste('FDA mutation approval= ', round(ROC_list$FDA_mutation_approval$auc[1],2)), # 'FDA mutation approval=0.91',
                  paste('SAB= ', round(ROC_list$SAB$auc[1],2)),
                  paste('Biogrid Highly Cited= ', round(ROC_list$Biogrid_Highly_Cited$auc[1],2)),
                  paste('DrugBank Active Inhibitors= ', round(ROC_list$DrugBank_Active_Inhibitors$auc[1],2)),
                  paste('DrugBank Active Antagonists= ', round(ROC_list$DrugBank_Active_Antagonists$auc[1],2)),
                  paste('Selective Inhibitors= ', round(ROC_list$Selective_Inhibitors$auc[1],2)))
...
<!-- platinum_dataset and finding the max specificities threshold from using gold standard dataset -->
```{r}
...
platinum_dataset = unique(Comp_Gold_Standard_df[platinum_dataset_ID,])
...
<!-- Figure 1E: Boxplot showing the data distribution of negative and positive labels in all datapoints and plantium datapoint. -->
```{r}
...
<!-- Figure 1F: Overall ROC across all data points and platinium datasets -->
```{r}
roc_overall_list = list(All_data_points = ROC_all_datapoints,
                      platinum = roc_platinum)
names(roc_overall_list) = c(paste('All data points = ',round(ROC_all_datapoints$auc[1],2)),
                            paste('Platinum Standard = ', round(roc_platinum$auc[1],2)))
...
<!-- Figure 1G: The relationship between the rank threshold of choice and specificity observed in the platinum-standard dataset is shown. -->
```{r}
roc_platinum = roc(platinum_dataset$Label, platinum_dataset$crispr_corr)
roc_platinum$specificities
...
Figure1G<- ggplot(Figure1E_df2plot, aes(x = rank_Threshold, y = Specificity) )+
  ...
  annotate(geom = "text", x = 5000, y = 0.2, label = "Rank Threshold\nat max specificity = 523", size = 4,
              color = "black")+
...
<!-- Attching all figure in one -->
```{r}
Figure1_withoutOverview =  grid.arrange(Figure1B, Figure1C, Figure1D,
                        Figure1E,Figure1F,Figure1G,
                        layout_matrix = rbind(c(1, 1, 2, 2, 3, 3), c(4, NA, 5, 5, 6, 6)))

ggsave(Figure1_withoutOverview, filename = '../result/Figure1_withoutOverview_v0.pdf',
       width = 12, height = 8)
```

## can't replicate the supplementary figure 1C, the rank is deviated by 1000. Also the roc_platinum$sensitivities are inf so, I am not understanding.
<!-- #Diamond dataset -->
<!-- Figure supplementary figure 1C -->
```{r}
...
```
```{r}
Comp_Gold_Standard_df[grep('cobimetinib',Comp_Gold_Standard_df$Chemical.Name, ignore.case = T),]
Comp_Gold_Standard_df[grep('cobimetinib',Comp_Gold_Standard_df$Chemical.Name, ignore.case = T),]
```
```{r}
df2plotFigure1H=data.frame(sensitivity=roc_platinum$sensitivities, specificities=roc_platinum$specificities)

df2plotFigure1H <- rbind(data.frame(rank_Threshold=roc_platinum$thresholds, Score=roc_platinum$specificities,Model_score='specificity'),
...
FigureS1C <- ggplot(df2plotFigure1H, aes(x=rank_Threshold, y=Score, color=Model_score))+
  ...
  geom_vline(xintercept = 5765, linetype='dashed', color='blue')+
  annotate(geom="text", x=5000, y=0.2, label="Rank Threshold\nat equal sensitivity\n & specivity=5765", size=4,
               color="black")+
  # theme(legend.position='none')+
  labs(x='Rank Threshold')
...
# ggsave(FigureS1C, filename = '../Results/FigureS1C.png',
#        width = 6, height = 4)
# ggsave(FigureS1C, filename = '../Results/FigureS1C.pdf',
#        width = 6, height = 4)
